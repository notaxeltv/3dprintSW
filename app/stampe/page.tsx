...
export default function StampePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-slate-900">Stampe</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="p-5 lg:col-span-1 h-fit">
          <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-slate-800">
            <Printer size={16} className="text-indigo-600" /> Nuova stampa
          </h2>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Card className="p-4">
              <p className="text-xs text-slate-500">Pezzi stampati totali</p>
              <p className="mt-1 text-xl font-semibold text-slate-900">{formatNumber(totalQuantity)}</p>
            </Card>
            <Card className="p-4">
              <p className="text-xs text-slate-500">Costo totale stampe</p>
              <p className="mt-1 text-xl font-semibold text-slate-900">{formatCurrency(totalCost)}</p>
            </Card>
          </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {logs?.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-slate-500">{formatDate(log.printedAt)}</td>
